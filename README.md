...
```sh
make docker-build docker-push IMG=<some-registry>/resource-controller-k8s-sigs:tag
```
...
```sh
make deploy IMG=<some-registry>/resource-controller-k8s-sigs:tag
```
...
```sh
make build-installer IMG=<some-registry>/resource-controller-k8s-sigs:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/resource-controller-k8s-sigs/<tag or branch>/dist/install.yaml
```
